...
export function ImageUpload(props) {
    ...
    const onImageSave = async (imageList) => {
        setImage(imageList);

        const formData = new FormData();
        formData.append('imagedata', imageList[0].file);
        formData.append('step_id_fk', props.stepidfk);
        formData.append('parent_step_name', props.parentstepname);
        formData.append('parent_step_id', props.parentstepid);
        ...
        try {
            const response = await OAuth2APIClient.post(
                `https://besterdev-api-13a0246c9cf2.herokuapp.com/api/v1/images/upload`,
                formData,
                {
    ...
    return (
        ...
        >
            {({ imageList, onImageUpload }) => (
                <>
                    {/* React Tooltip definition */}
                    <Tooltip id="image-upload-tooltip" place="top" />

                    <button
                        type="button"
                        onClick={onImageUpload}
                        data-tooltip-id="image-upload-tooltip"
                        data-tooltip-content="Add or Replace an image to this Step"
                        style={{
                            height: '20px',
                            width: '20px',
                            padding: 0,
                    ...
                    {imageList.map((img, index) => (
                        <img key={index} src={img.data_url} alt="preview" />
                    ))}
                </>
            )}
        </ImageUploading>
