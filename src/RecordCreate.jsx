...
export default function RecordCreate(props) {
  ...
  const handleSubmit = async (event) => {
    ...
    console.log(newRecord)
      ...
      {const response = await axios.post(`https://besterdev-api-13a0246c9cf2.herokuapp.com/api/v1/records/create`, newRecord);
        ...
        {props.setCheckForRecords(!props.checkForRecords); alertCtx.success(`${colone} memorialized`);}

        else {alertCtx.error(`oops! Something went wrong!`);}
      }

      catch(err) 
      {alertCtx.error(`oops! Something went wrong!`); console.log(err);}
    }
    else {
      event.preventDefault();
  ...
  return (
    ...
    <div className='Font-Verdana-Small'>
      ...
      <div onClick={toggleAccordion}><img src={spacer} alt="1"/><a data-tooltip-id="insert" data-tooltip-content="Insert"><GiHummingbird style={{color: '#1994AD', fontSize: '35px', cursor: 'pointer' }}/></a><b>Add a Record</b><img src={spacer} alt="1"/><a data-tooltip-id="insert" data-tooltip-content="Insert"></a></div>
      {isExpanded && (
        <div>
        <div>
