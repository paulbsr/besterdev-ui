...
import ImageDH from '../graphix/dh1.jpg' //Lady Liberty
...
export default function DHKeyExchange(props) {
    ...
    const [dhparameterslatest, setDhparameterslatest] = useState('');
    const [checkForRecords, setCheckForRecords] = useState(true);

    useEffect(() => {
    axios(`https://besterdev-api-13a0246c9cf2.herokuapp.com/api/v1/dh/parameters/retrieve/latest`)
      .then((response) => {
        const dhparameterslatest = response.data;
        setDhparameterslatest(dhparameterslatest);
      })
      .catch((error) => {
        console.error("Error fetching dhparameterslatest data:", error);
      });
  }, [checkForRecords]);
            ...
            <table style={{ width: '100%' }}>
                <thead>
                    <tr>
                        ...
                        <td style={{ width: '1%' }}></td>
                        <td style={{ width: '68%' }}><img src={ImageDH} /></td>
                        <td style={{ width: '1%' }}></td>
                        <td style={{ width: '15%' }}></td>
                    </tr>
                </thead>
                ...
                <tbody>
                    <tr>
                        ...
                        <td style={{ width: '1%' }}></td>
                        <td style={{ width: '68%' }}>
                            {dhparameterslatest.length > 0 ?
                                (
                                    dhparameterslatest.map((item) => (
                                        <div key={item.id}>
                                                    ...
                                                    editing ?
                                                        (
                                                            <>
                                                                ...
                                                                <i>Diffie-Hellman Parameters (P, G, L) are used to generate the public-private Key Pairs.:</i>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                                                {/* <input
                                                                    required
                                                                    defaultValue={item.p}
                                ...
                                :
                                (
                                    <div>The Diffie-Hellman Key Exchange was created to solve a fundamental problem in cryptography:
👉 How can two parties securely agree on a shared secret over an insecure communication channel without having previously shared a secret key?

Why it was needed
Before Diffie-Hellman (introduced in 1976 by Whitfield Diffie and Martin Hellman):

Cryptographic systems mostly relied on symmetric encryption, which means both parties had to know the same secret key.

The biggest issue was key distribution: if two people wanted to communicate securely, they first had to meet in person or use a trusted courier to share the secret key — which is risky, slow, or impractical on a large scale.

What Diffie-Hellman introduced
Diffie-Hellman introduced the concept of public key exchange, which allowed:

Two parties to agree on a shared secret key over an insecure channel.

The shared key could then be used for symmetric encryption of their messages.

It did this using mathematical principles from modular arithmetic and the discrete logarithm problem, which is computationally hard to reverse.</div>
                                )
                            }
                        </td>
                        <td style={{ width: '1%' }}></td>
