                ...
                {tabledata.map((row) => {
                  return (
                    <tr key={row.howto_id}>
                      ...
                      <td className="asmshover Table6 td">{row.howto_id === editing ? (<input style={{ height: '22.5px', width: '90px', border: '1.25px solid #336791', borderRadius: '4px', padding: 0, paddingLeft: '5px' }} value={howto_cat} onChange={(e) => setHowto_cat(e.target.value)} />) : (row.howto_cat)}</td> 
                    </tr>
                  )
                })
                }
