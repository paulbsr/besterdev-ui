...
const handleSubmitChatbot = async (e) => {
  ...
  try {
    ...
    const createRes = await OAuth2APIClient.post(API_BASE, {
      userInput: entry,
      botResponse: "",
      botCorrection: "",
      grammarScore: 0,
    ...
    // Axios automatically throws for HTTP errors (unless configured otherwise)
...
- botResponse (conversational about the topic, not linguistic feedback, always in Dutch, encourage responses to be in the future tense or past tense)`
    ...
    const aiRes = await OAuth2APIClient.post(AI_ENDPOINT, aiPrompt, {
      headers: { "Content-Type": "application/json" }
    });
...
const handleSubmitDiary = async (e) => {
  ...
  try {
    ...
    const postRes = await OAuth2APIClient.post(API_BASE,
      { myEntry: entry, aiEntry: "", feedback: "" },
      { headers: { "Content-Type": "application/json" } }
    );

    const savedId = postRes.data.id;
    ...
    const aiRes = await OAuth2APIClient.post(AI_ENDPOINT, {
      question: `
    You are a Dutch language teacher. A student wrote: "${entry}". Please respond ONLY in raw JSON with keys "feedback" and "suggestedSentence" and "score".
    Scoring rules:
    5 = Excellent, no or tiny mistakes
