    ...
    <div style={{ marginBottom: "6px" }}>
      ...
      {expanded && (
        ...
        >
          ...
          >
            Word Order: {item.scoreWordorder} , Grammar: {item.scoreGrammar} ,
            Vocab: {item.scoreVocabulary} , Spelling: {item.scoreSpelling} ,
            Comprehensibility: {item.scoreComprehensibility} , Nouns:{" "}
            {item.scoreNoun} , Articles: {item.scoreArticle} , AVG:{" "}
            {item.scoreUserAverage} / 5
          </div>

          {item.aiCorrection && (
            <div style={{ color: "#FF4F00", marginBottom: "6px" }}>
              {item.aiCorrection}
            </div>
          )}
          {item.aiFeedback && (
      ...
      {expanded && (
        <div style={{ marginLeft: "15px", marginTop: "5px" }}>
          {entries
            ...
            .sort(
              (a, b) =>
                new Date(b.createdAtParsed) - new Date(a.createdAtParsed)
            )
            .map((item) => (
              <TimeGroup key={item.id} item={item} />
            ))}
...
export default function DutchLanguage_Chatbot() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const rec = {
        ...updated,
        createdAtParsed: updated.createdAt
          ? parseCreatedAt(updated.createdAt)
          : new Date(),
      };
  ...
  const groupedByDate = allEntries
    .slice()
    .sort(
      (a, b) => new Date(b.createdAtParsed) - new Date(a.createdAtParsed)
    )
    .reduce((acc, item) => {
      const d = new Date(item.createdAtParsed);
  ...
  return (
    ...
    >
      ...
      <h2 style={{ fontWeight: "bold", fontSize: "22px", margin: 0 }}>
        ...
        />
        Nederlandse AI Chatbot
      </h2>

      {/* --------------------------- */}
      {/* USER TEXTAREA + WORD COUNT  */}
      {/* --------------------------- */}
      <form onSubmit={handleSubmit}>
        <textarea
          value={entry}
      ...
      >
        <GiAngryEyes
          style={{ fontSize: "32px", color: "#000000", marginRight: "8px" }}
        />
        <span style={{ fontSize: "12pt" }}>
          {showPastEntries
            ? "Verberg eerdere chatberichten"
            : "Bekijk eerdere chatberichten"}
        </span>
      </div>
      ...
      {showPastEntries && (
        <div style={{ marginTop: "12px" }}>
          {Object.keys(groupedByDate).length === 0 && (
            <div style={{ color: "#999" }}>Geen eerdere chatberichten.</div>
          )}
